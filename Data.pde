...
public class Data {
  ...
  void loadData() {
    table = loadTable("hygdata_v3.csv", "header");          //This is processings way of loading a csv file
  ...
  void merge(int start, int mid, int end) {
    ...
    //temporarily store the array that is obtained when elements are merged from
    ...
    while (i < n1) {                  //When we run out of elements in first half but elements still exist in first half, add the first half of elements into temporary array
      starDist.set(k, L.get(i));
      i++;
      k++;
    }

    while (j < n2) {                  //When we run out of elements in second half but elements still exist in first half, add the first half of elements into temporary array
      starDist.set(k, R.get(j));
      j++;
      k++;
    }
